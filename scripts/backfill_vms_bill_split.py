...
def main():
    ...
    # ── ด่านความปลอดภัย: ยอดรวมรายวันต่อตู้ต้องไม่ขยับอย่างมีนัย ──
    #
    # ⚠️ ยอมให้ขยับได้ระดับสตางค์ **โดยตั้งใจ** — การกู้ยอดบิลกลับเป็นจำนวนเต็ม
    #    (SNAP_MAX) ทำให้วันที่มีบิลถูกกู้ 2 ใบขยับได้ ±0.02 บาท ซึ่งเป็นการแก้ให้
    #    *ถูกขึ้น* ไม่ใช่เงินหาย · ของเดิมที่ปัดทีละบรรทัดต่างหากที่ผิด
    #
    # ⚠️ ต้อง round() ก่อนเทียบ — ทศนิยมลอยตัวทำให้ 10569.98→10570.00 ออกมาเป็น
    #    0.020000000000437 ซึ่ง > 0.02 แล้วด่านจะฟ้องทั้งที่ไม่มีอะไรผิด (เจอจริง 2 ก.ย. 2026)
    DAY_TOL = 0.05          # 5 สตางค์/ตู้/วัน = กู้ยอดได้ถึง 5 บิลในวันเดียว
    bad_days, satang_shift = [], 0.0
    for k in set(before_day) | set(after_day):
        diff = round(after_day[k] - before_day[k], 2)
        if abs(diff) > DAY_TOL:
            bad_days.append((k, before_day[k], after_day[k]))
        elif diff:
            satang_shift += diff
    if satang_shift:
        print(f"ยอดรวมขยับระดับสตางค์จากการกู้ยอดบิล: {satang_shift:+.2f} บาททั้งช่วง "
              f"(ยอมรับได้ · เพดาน {DAY_TOL} บาท/ตู้/วัน)")
    if bad_days:
