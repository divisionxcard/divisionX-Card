---
type: machine
machine_id: wwv04
brand: worldwide
machine_id_vendor: VCM350CKC25120001
status: active
location: เซ็นทรัล เวสต์เกต
route: บางใหญ่
version: SXA1B31R.THA230915.013
last_updated: 2026-06-03
---

# ตู้ที่ 8 (wwv04) · เซ็นทรัล เวสต์เกต

ตู้ WorldWide Vending ติดตั้งใหม่ 2026-06-03 — ดู [[2026-06-03-add-ww-machines]]

## ข้อมูลพื้นฐาน

| Field | Value |
|-------|-------|
| Machine ID | wwv04 |
| Brand | worldwide |
| Vendor ID | VCM350CKC25120001 |
| Route | บางใหญ่ |
| ที่ตั้ง | เซ็นทรัล เวสต์เกต |
| Version | SXA1B31R.THA230915.013 |
| สถานะ | ใช้งาน (active) |

## 📦 สินค้าในตู้ (Current Slots)

_scraper จะ sync จาก machine_stock (00:15 น. เวลาไทย)_

## 📝 หมายเหตุ

- เพิ่มเข้าระบบผ่าน migration `042_add_worldwide_wwv03_wwv04.sql`
- WorldWide เป็น data-driven — sync อัตโนมัติจากตาราง `machines` ไม่ต้องแก้ scraper
- ตู้พี่น้อง: [[wwv01]] · [[wwv02]] · [[wwv03]]
